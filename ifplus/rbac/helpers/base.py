...
class LdapConfig(object):
    def __init__(self):
        ...
        self.OBJECT_CLASSES = copy.deepcopy(REGISTER_OBJECT_CLASSES)
        self.ENTITY_CLASSES = {}
...
class LdapConnection(object):
    ...
    def find_by_iid(self, base_dn, iid):
        try:
            result = self.conn.search_s(base_dn, ldap.SCOPE_SUBTREE, '(%s=%s)' % (FT_IID, iid))
        except ldap.NO_SUCH_OBJECT:
            return None
        if result is not None and len(result) > 0:
            dn, attrs = result[0]
...
class BaseHelper(object):
    ...
    def instance(self, dn=None, attrs=None, **kwargs):
        ...
        if cls.ID_FIELD not in entry.attrs:
            entry.attrs[cls.ID_FIELD] = rdn(entry.dn)
        return entry

    def getattr(self, entry, attr_name):
        if attr_name == 'name':
            return get_first(entry.attrs.get(entry.ID_FIELD, None))
        ...
        ft_attr_name = FT_PREFIX + attr_name.title()
        if ft_attr_name in self.ldap.config.ENTITY_CLASSES[class_name]['ALL']:
            if attr_name == 'id':
                return get_first(entry.attrs[ft_attr_name])
            else:
    ...
    def create(self, entry):
        ...
        class_name = self.entity_class().__name__
        if FT_IID in self.ldap.config.ENTITY_CLASSES[class_name]['MUST']:
            entry.iid()
        entry.fill()
